...
public class Mesh {
    ...
    public static Mesh createUvSphere(
            ...
            int mediaFormat) {
        float[] vertexData = getVertexData(radius, latitudes, longitudes, verticalFovDegrees, horizontalFovDegrees, mediaFormat);

        return new Mesh(vertexData);
    }

    protected static float[] getVertexData(float radius, int latitudes, int longitudes, float verticalFovDegrees, float horizontalFovDegrees, int mediaFormat) {
        if (radius <= 0
    ...
    public void glDraw(float[] mvpMatrix, int eyeType) {
        ...
        // Render.
        GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP, 0, verticesLength / CPV);
        checkGlError();
